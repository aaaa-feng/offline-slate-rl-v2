#!/bin/bash
# Group B: mf_init KL=0.05 IQL
# 对比 old mf_init KL=1.0 (beta_ablation_repreduce)
#
# Usage:
...
run_one() {
    ...
    local ckpt="${CKPT_DIR}/GeMS_${env}_b5_pretrained_latent32_beta0.05_click1.0_seed58407201_mf_init.ckpt"
    ...
    mkdir -p logs/agents/kl005_mf_init

    CUDA_VISIBLE_DEVICES=${gpu} nohup ${PYTHON} -u scripts/train_agent.py \
        --config experiments/action_cloud/kl005_iql/mf_init/config.yaml \
        --env_name ${env} \
        --beta ${beta} \
        --gems_checkpoint ${ckpt} \
        --experiment_name "kl005_mf_init/${label}" \
        --experiment_tag "kl005_mf_init/${label}" \
        > logs/agents/kl005_mf_init/${label}.log 2>&1 &
...
echo ""
echo "Launched. Logs: logs/agents/kl005_mf_init/"
echo "Checkpoints: checkpoints/agents/kl005_mf_init/{mix,td}_b{N}/"