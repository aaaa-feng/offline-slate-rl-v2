#!/usr/bin/env python3
"""Policy latent (action z) PCA panels: grey = online step-999, color = reward.

Usage:
    python experiments/online_sac_geometry/plot_action_panels_reward.py
"""
...
def load_action_reward(run_dir: Path, step: int) -> tuple[np.ndarray, np.ndarray]:
    path = npz_path(run_dir, step)
    if path is None:
        raise FileNotFoundError(f"missing step {step}")
    data = np.load(path, allow_pickle=True)
    return (
        np.asarray(data["policy_latent_raw"], dtype=np.float32),
...
def plot_panels(
    ...
    cloud999, _ = load_action_reward(run_dir, BASELINE_STEP)
    entries = []
    for step in steps:
        action_z, reward = load_action_reward(run_dir, step)
        entries.append({"step": step, "action_z": action_z, "reward": reward})
    ...
    for ax, entry in zip(axes[0], entries):
        ...
        xy = pca.transform(entry["action_z"])
        sc = ax.scatter(
            xy[:, 0], xy[:, 1],
    ...
    fig.suptitle(
        f"{run_label} — policy latent (action z) checkpoint panels (reward)\n"
...
def main() -> None:
    ...
    plot_panels(
        run_dir,
        out_dir / "trajectory_panels_action_reward.png",
        steps,
        args.run_label,
    )
