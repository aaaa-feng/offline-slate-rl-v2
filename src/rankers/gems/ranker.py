...
from .embeddings import ItemEmbeddings
...
class AbstractGeMS(Ranker):
    ...
    def training_step(self, batch, batch_idx) -> torch.FloatTensor:
        ...
        with torch.no_grad():
            per_dim_std = latent_mu.std(dim=0).mean()
            embed_weights = self.item_embeddings.weight.data
            embed_norm_mean = embed_weights.norm(dim=1).mean()
            embed_norm_std = embed_weights.norm(dim=1).std()
        ...
        self.log("train_latent_per_dim_std", per_dim_std)
        self.log("train_embed_norm_mean", embed_norm_mean)
        self.log("train_embed_norm_std", embed_norm_std)
    ...
    def validation_step(self, batch, batch_idx) -> torch.FloatTensor:
        ...
        # Per-dim std + SVD rank (KL ablation 核心指标)
        with torch.no_grad():
            per_dim_std = latent_mu.std(dim=0).mean()
            self.log("val_latent_per_dim_std", per_dim_std)

            # SVD effective rank (80% variance)
            if latent_mu.size(0) >= 32:
                U, S, V = torch.svd(latent_mu.float())
                ev = S ** 2
                evr = ev / (ev.sum() + 1e-8)
                cs = torch.cumsum(evr, 0)
                pc80 = torch.searchsorted(cs, 0.8).item() + 1
                self.log("val_latent_svd_pc80", float(pc80))

            # Embedding cosine similarity
            e = self.item_embeddings.weight.data
            e_n = e / (e.norm(dim=1, keepdim=True) + 1e-8)
            cos_mat = e_n @ e_n.T
            mask = ~torch.eye(len(e), dtype=torch.bool, device=e.device)
            cos_sim_mean = cos_mat[mask].mean()
            self.log("val_embed_cos_sim_mean", cos_sim_mean)
